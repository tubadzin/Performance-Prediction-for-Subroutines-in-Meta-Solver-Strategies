...
dr = d;
if strcmp(func2str(funObj),'ReducedFunObj')
    varargin{1};
    d = varargin{1}*d;
elseif strcmp(func2str(funObj),'barrierFunObj')
    A = varargin{2};
    B = varargin{3};
    % Truncate Step to insure feasibility
    while any(isinf(log(B-A*(x+t*d)))) || any(~isreal(log(B-A*(x+t*d)))) && t > tolX
        t = t*.9;
    end
end
...
while f_new > fr + c1*t*gtd || ~isLegal(f_new)
    ...
    elseif LS == 2 && isLegal(g_new)
        ...
        t = polyinterp([0 f gtd; t f_new g_new'*dr],doPlot);
