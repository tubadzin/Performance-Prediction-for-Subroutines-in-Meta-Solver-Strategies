...
void lbfgsUpdate(const int mrowsY, const int numCorrections, const double* y, const double* s, const int corrections, const int debug, const double* old_dirs, const double* old_stps, double* old_dirs_out, double* old_stps_out, double* Hdiag_out){
    ...
    if (y_times_s > 1e-10){
        if( numCorrections == corrections ){
            ...
            for( c=0; c<numCorrections-1; c++ ){
                for( i=0; i<mrowsY; i++ ){
                    old_dirs_out[c*mrowsY + i] = old_dirs[(c+1)*mrowsY + i];
                    old_stps_out[c*mrowsY + i] = old_stps[(c+1)*mrowsY + i];
                }
            }
            num_columns = numCorrections;
        } else {
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    double *old_dirs_out, *old_stps_out, *Hdiag_out;
    int corrections, debug, mrowsY, mrows, ncols, numCorrections, i;
    ...
    /* Hdiag is 7'th input, but is not used. */
    ...
    lbfgsUpdate(mrowsY,numCorrections,y,s,corrections,debug,old_dirs,old_stps,old_dirs_out,old_stps_out,Hdiag_out);
}