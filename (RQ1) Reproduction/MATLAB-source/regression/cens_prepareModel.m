function [model, nlml, dnlml] = cens_prepareModel(model, invKmm)
assert(all(isfinite(model.y)));

switch model.type
    case {'rf', 'LR', 'ivm', 'regression-tree', 'javarf', 'fastrf'}
    otherwise
...
%         Splitmincens = max(1, ceil(tuning_params(3)*model.options.splitMinMax-0.5));
        ...
        if hascens
%{
%             fprintf('Model has censored data - using survival forest\n');
...
%}            
            %=== Initialize indices.
            noncens_idx = find(model.cens==0); % first iteration: noncens
            cens_idx = find(model.cens==1);
%             if isempty(noncens_idx) % if all censored predict max. value
%                 model.constant = true;
%                 if model.options.logModel
%                     model.y(1:end) = log10(model.options.kappa_max * model.options.cutoff_penalty_factor);
%                 else
%                     model.y(1:end) = model.options.kappa_max * model.options.cutoff_penalty_factor;
%                 end
%                 return;
%             end
            ...
            %% ============ Impute values. =========
            mu = model.censMeanPred;
            sigma = sqrt(model.censVarPred);

            if strcmp(model.options.censoring, 'fill_in_mean')
                %== Compute values to be imputed everywhere
                %== (expectation of truncated Gaussian)
                for i=1:length(mu)
                    alpha = (model.y(cens_idx(i))-mu(i))/sigma(i);
                    single_y_hal(cens_idx(i),1) = mu(i) + sigma(i) * normpdf(alpha) / (1-normcdf(alpha));
                end
                single_y_hal = min(single_y_hal, log10(model.options.kappa_max * model.options.cutoff_penalty_factor));
                model.mean_imputed = single_y_hal(cens_idx);
                for m=1:M
%                     this_r = r(:,m);
%                     yHal{m} = single_y_hal(this_r);
                    for i=1:length(cens_idx)
                        idxs = find(r(:,m)==cens_idx(i));
                        if ~isempty(idxs)
                            yHal{m}(idxs,1) = single_y_hal(cens_idx(i),1);
                        end
                    end
                end

            elseif strcmp(model.options.censoring, 'fill_in_samples')
                %== Compute different values to be imputed everywhere
                %== (samples of truncated Gaussian)

%                 old_prng_state = rand('twister');
%                 rand('twister', seed_to_fill_in_samples);

                numOccurrence = zeros(M, 1);

                for i=1:length(cens_idx)
                    %= Count how many samples we need.
                    totalOccurrence = 0;
                    for m=1:M
                        numOccurrence(m) = length(find(r(:,m) == cens_idx(i)));
                        totalOccurrence = totalOccurrence + numOccurrence(m);
                    end
                    if totalOccurrence == 0
                        continue;
                    ...
                    %= Get the samples.
                    inc = 1/(totalOccurrence+1);
                    u = inc:inc:1-inc;
                    perm = randperm(totalOccurrence);
                    u = u(perm);
                    samples = rand_draw_truncated_normal(model.y(cens_idx(i)), inf, mu(i), sigma(i), [1 totalOccurrence], u);
%                         [other_samples, eff_mu, eff_sigma] = TruncatedGaussian(-sigma(i), [model.y(cens_idx(i)),inf]-mu(i), [1 totalOccurrence]);
%                         other_samples = samples + mu(i);
%                         eff_mu = eff_mu + mu(i);
%                         assert(eff_mu >= mu(i) - 1e-6, 'mu cannot get smaller by conditioning on being above the censoring threshold');
%                         assert(abs(eff_mu-mean(samples)) <= 1e-6, 'mu cannot get smaller by conditioning on being above the censoring threshold');
                    sample_mean = mean(samples);
                    max_avg = log10(model.options.kappa_max * model.options.cutoff_penalty_factor);
                    if isfinite(sample_mean)
                        if sample_mean > max_avg
                            samples = samples - sample_mean + max_avg;
                        end
                    else
                        samples = min(samples, max_avg);
                    end
                    ...
                    for m=1:M
                        idxs = find(r(:,m)==cens_idx(i));
                        if ~isempty(idxs)
                            yHal{m}(idxs,1) = samples(sample_idx:sample_idx+length(idxs)-1);
                            sample_idx = sample_idx + length(idxs);
%                             mean_imputed = mean_imputed + sum(yHal{m}(idxs))/(M*length(cens_idx));
                        end
                    end
                    assert(sample_idx == totalOccurrence+1);
                    mean_imputed(i,1) = mean(samples);
                end
                model.mean_imputed = mean_imputed;
%                 rand('twister', old_prng_state);
            else
                error(['unknown censoring option: ', options.censoring]);
            end
            ydata = yHal;

%                 average_increase_in_imputed_values = mean_imputed - old_mean_imputed
%                 if average_increase_in_imputed_values < 1e-10 && imputation_iteration >= 2
%                     fprintf(['Mean of imputed values stopped increasing in iteration ', num2str(imputation_iteration), '.']);
%                     return;
%                 end

                %=== Refit the RF with hallucinated values.
%                 for m=1:M
%                     model.yHal{m} = yHal{m};
%                     this_r = r(:,m);
%                     Xsub = model.X(this_r, :);
...
% 
%                     seed = ceil(rand*10000000);
%                     model.module{m}.T = fh_simple_random_regtreefit_algoParams_instFeats_c(Xsub, model.yHal{m}, censsub, Splitmin, ratioFeatures, model.cat, model.catDomains, seed, model.options.kappa_max, model.options.logModel, model.options.overallobj);
%                 end
%             return

        else 
            fprintf('Model has no censored data - using normal random forest\n');
            %=== Just do "normal" RF.
            ydata = {};
            for m=1:M
                ydata{m} = model.y(r(:, m));
        ...
        for m=1:M
			...
			if isjavarf
				...
				Xsub = model.X(this_r, :);
                model.module{m}.T = cens_fh_simple_random_regtreefit_algoParams_instFeats_c(Xsub, ysub, censsub, Splitmin, ratioFeatures, model.cat, model.catDomains, seed, model.options.kappa_max, model.options.logModel, model.options.overallobj);
			end
        end
        if isjavarf
...
%         
%         savename = 'saved_workspace.mat';
%         fprintf(['Error building model. Saving workspace to ', savename, '\n']);
%         save(savename);
%         ME.rethrow();
%     end
end
