function Tree=cens_fh_simple_random_regtreefit_algoParams_instFeats_c(X,y,cens,Splitmin,percentageFeatures,Catidx,domains_cat,seed,kappa,logModel,overallobj)
    ...
    leftchildren_m, rightchildren_m, resuberr_m, nodesize_m, catsplit_m, numNodes_m, ncatsplit_m] ...
        = cens_fh_random_regtreefit_big_leaves_twofeaturetypes_dist_part( ...
...
% FH, Sept 19: that is wrong. We want to calculate mean/var across trees based on log values.    
% FH, Sept 20: we only saved these nonlog values in the tree for applyMarginalModel. 
%     if logModel
...
%     end
    ...
    Tree = cens_maketree_ysub_dist(X, find(iscat==1), nodenumber_m, parent_m, ysub_m, censsub_m, cutvar_m, cutpoint_m, ...
