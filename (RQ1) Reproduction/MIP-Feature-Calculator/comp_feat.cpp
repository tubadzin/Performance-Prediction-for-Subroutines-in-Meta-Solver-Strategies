  ...
  /* The following are really integers, but to avoid having to rewrite computation of statistics for ints, we make them doubles. */
    ...
    ****************************************************************************/
	...
	/* If variable k_all is not an element of the set we're currently considering then skip it. */
      	...
      	// BUGFIX: the original code wrote “fabs(rhs_b[i] > 1e-6)”
      	// which takes the absolute value of a boolean (0 or 1), not of the RHS itself.
      	// That meant rows with a small nonzero RHS were treated as zero (or vice versa),
      	// so A_ij_normalized never got populated properly (leaving default –512 values).
      	// The correct test is “fabs(rhs_b[i]) > 1e-6” to check the magnitude of the RHS.
...
int main (int argc, char *argv[]){
  /* Declare variables. */
    CPXENVptr     env = NULL;
    CPXENVptr     env_relax = NULL;
    CPXLPptr      lp = NULL;
    CPXLPptr      lp_relax = NULL;
    int probtype;
    ...
    initialize(argv[1], debug, &env, &lp);
    initialize(argv[1], debug, &env_relax, &lp_relax);
