function options = cens_get_rf_default_options
options.modelType = 'rf';
options.nSub = 10;
options.logModel = 1;
options.pca = -1; % do no PCA here, for feature importance
options.orig_rf = 0;
options.init_split_ratio = 0.5; % for modelling paper only
options.init_Splitmin = 5; % for modelling paper only
options.orig_rf = 0;
...
options.unique_model_name = 'RF-def-cens';

options.opt = 0;
options = fill_in_options(options);