function model = cens_internalLearnModel(Theta_inst_idxs, ThetaUniqSoFar, all_instance_features, y, cens, thetaCat, thetaCatDomains, xCat, xCatDomains, cond_params_idxs, parent_param_idxs, ok_parent_value_idxs, isclean, options, names, censMeanPred, censVarPred, varargin)
if nargin < 17
    censMeanPred = [];
    censVarPred = [];
    if nargin < 15
...
fprintf(strcat(['Learning model with ', num2str(size(Theta_inst_idxs,1)), ' data points of dimension ', num2str(size(Theta, 2) + size(pcaed_instance_features, 2)),' ... \n']));

%% === Wrapper around initializing a model and then preparing it for prediction.
model = cens_initModel(Theta_inst_idxs, ThetaUniqSoFar, pcaed_instance_features, y, cens, thetaCat, thetaCatDomains, xCat, xCatDomains, cond_params_idxs, parent_param_idxs, ok_parent_value_idxs, options, names);
model.censMeanPred = censMeanPred;
model.censVarPred = censVarPred;
...
%     model.params = [-2;3];
        ...
        model = cens_optimizeModel(model, varargin{:});
    end
end

model = cens_prepareModel(model);
