function Tree=fh_treefit(X,y,varargin)
...
% %   $Revision: 1.2 $  $Date: 2010/11/11 21:21:01 $
% 
% % Process inputs
% if isnumeric(y)
%    Method = 'regression';
...
% %  .proportionleft probability of going left at this node
...
%       mincost = sst / Nnode;
%       impure = (mincost > 0);%1e-6 * resuberr(1));
%    end
%    bestcrit          = -Inf;
...
%       randomPermutation = randperm(nvars);
%       for jvar=randomPermutation(1:nvars)%1:nvars 
...
%             [critval,cutval]=Rcritval(x,ycum,rows,xcat); % x:sorted jth variable, ycum above, rows: idx of change in x, xcat: whether x is cat
%          end
% 
...
%             leftside = x<=bestcut;
%             rightside = ~leftside;
%             proportionleft(tnode) = (length(leftside)/ (0.0+length(leftside)+length(rightside)));
%          else
...
% Tree.proportionleft = proportionleft(1:topnode);
...
% % x:sorted jth variable, Ycum: centered cum. response, rows: idx of change in x, iscat:
% % whether x is cat
% 
% % First get all possible split points
% 
